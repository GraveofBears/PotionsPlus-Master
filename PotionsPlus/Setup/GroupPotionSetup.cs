...
public static class GroupPotionSetup
{
    ...
    {
        // Check if Groups API is loaded before setting up group potions
        bool groupsLoaded = API.IsLoaded();
        if (!groupsLoaded)
        {
            Debug.Log("[PotionsPlus] Groups mod not detected - group potions will be created but may not work properly");
        }
