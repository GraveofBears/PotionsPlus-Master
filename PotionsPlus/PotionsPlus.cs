...
public class PotionsPlus : BaseUnityPlugin
{
    ...
    public void Awake()
    {
        ...
        AssetBundle assets = potionAssets;
        ...
        grandHealingTidePotionCooldown = config("Grand Healing Tide Potion", "Cooldown Duration", 240, new ConfigDescription("Cooldown of the Grand Healing Tide Potion in seconds.", new AcceptableValueRange<int>(1, 600)));
        // Grand Stamina Elixir
        ...
        hellbrothOfEternalLifeTTL = config("Hellbroth of Eternal Life", "Effect Duration", 5f, new ConfigDescription("Duration of the healing effect in seconds.", new AcceptableValueRange<float>(1f, 30f)));
        hellbrothOfEternalLifeRadius = config("Hellbroth of Eternal Life", "Healing Radius", 5f, new ConfigDescription("Healing radius in meters.", new AcceptableValueRange<float>(1f, 20f)));
        hellbrothOfEternalLifeTickInterval = config("Hellbroth of Eternal Life", "Heal Interval", 1f, new ConfigDescription("Time between healing ticks in seconds.", new AcceptableValueRange<float>(0.25f, 5f)));
        // Brew of Faint Group Healing
    ...
    private void Start()
    {
        // ALL AddPlaceholder calls corrected to match localization keys
    ...
    public class HealingAoe : MonoBehaviour
    {
        ...
        private void HealNearbyCharacters()
        {
            ...
            // Find all colliders in radius on the character layer
            int layerMask = LayerMask.GetMask("character", "character_noenv", "character_ghost");
            ...
            foreach (Collider col in hitColliders)
            {
                ...
                // Prevent healing same character multiple times per tick (if they have multiple colliders)
                int charId = character.GetInstanceID();
                if (healedThisTick.Contains(charId))
                {
                    continue;
                }
                healedThisTick.Add(charId);

                // Check if this is a friendly target
                bool shouldHeal = character is Player || character.IsTamed();

                if (shouldHeal)
                {
                    // Calculate heal amount per tick
                    float maxHealth = character.GetMaxHealth();
                    float healAmount = maxHealth * hellbrothOfEternalLifeHealing.Value / 100f;

                    // Apply healing
                    character.Heal(healAmount);
                    healedCount++;

                    // Show message for players (but not spam every second)
                    if (character is Player player && Random.value < 0.3f) // Only show 30% of the time
                    {
